function addEle(name, pre_item, suc_item, last_time, earliest_start_time, earliest_finish_time, latest_start_time, latest_finish_time, free_time_difference, total_time_difference, is_key) {
    ...
    if (is_key == true) {
        $row.css('backgroundColor', 'rgba(159, 189, 255, 0.52)');
    }
}
    ...
    $.ajax({
        ...
        success: function (items) { // 这里的data就是json格式的数据
            items.forEach(function (v) {
                addEle(v.item_name, v.item_pre_item, v.item_suf_item, v.item_last_time, v.item_earliest_start_time, v.item_earliest_finish_time, v.item_latest_start_time, v.item_latest_finish_time, v.item_free_time_difference, v.item_total_time_difference, v.item_is_key);
            });
